...
class PKpassOrg extends PKpass {
    /**
    ...
    public function __construct($certPath = false, $certPass = false, $JSON = false)
    {
        ...
        $this->tempPath = '/var/www/html/tmp/';
...
print_r($a = "$this->tempPath" . "$this->uniqid");
print_r($this->controller);
#$this->controller->set(compact('a'));
...
public function aaa()
{
return "$this->tempPath" . "$this->uniqid";
}
    /**
     * @return mixed
     */
...
#$a = var_dump($this->json);
    ob_start();
    print_r($this->json);
    $result =ob_get_contents();
    ob_end_clean();
 file_put_contents("/var/www/html/tmp/sample1.txt", $result);
    #$fp = fopen("/var/www/html/tmp/sample1.txt", "a+" );
    #fputs($fp, $result);
    #fclose( $fp );
